...
Copy `scripts/OutlookKeywordSearch_PS.ps1` to a local folder, e.g.:
```
...
3. Paste the full contents of `scripts/OutlookKeywordSearch_PS.bas`
...
|---------|-------|---------|
| PS script path | `PS_SCRIPT_PATH` in VBA module | `C:\Users\Varun\scripts\OutlookKeywordSearch_PS.ps1` |
...
| Version | Summary |
|---------|---------|
| v1 | Initial PS-assisted release. Extracted from combined script. VBA launcher + PS engine. Background execution, toast notifications, log file. Non-mail folder skip, early exit per folder, OS yield between batch keywords. |
