...
.DESCRIPTION
    ...
    Optimizations:
      - Skip non-mail folders (Calendar, Contacts, Tasks, Junk, Deleted, etc.)
      - Early exit per folder once current item is newer than best match found
      - Yields to OS between keywords in batch mode
      - Runs as a separate PS process — Outlook UI never blocked
...
if ($Mode -eq 'B') {
    ...
    $summary = "Batch complete. Processed: $processed | Found: $found"
    Write-Log $summary
