...
| `OutlookKeywordSearch_Standalone.bas` | [Testing Readme](OutlookKeywordSearch_Standalone.bas%20Testing%20Readme.md) | — | Manual |
| `OutlookKeywordSearch_PS.bas` | [Testing Readme](OutlookKeywordSearch_PS.bas%20Testing%20Readme.md) | — | Manual |
...
3. Browse to the `.bas` file and click **Open**.
...
|---------|-------------|-------|
| **Microsoft Scripting Runtime** | `ExcelFormatting.bas`, `SplitExcelByManager.bas` | Enables `Scripting.Dictionary` and `FileSystemObject`. |
| **Microsoft Word 16.0 Object Library** | Word macro scripts | Required when automating Word from Excel VBA. |
| **Microsoft Excel 16.0 Object Library** | Word macro scripts that write back to Excel | Required when Excel object model needed from outside Excel. |
| **Microsoft Office 16.0 Object Library** | Any script using shared Office objects | Usually checked by default. |
...
- **From the ribbon:** Developer → Macros → select macro → **Run**.
- **From the VBE:** press **F5** to run, **F8** to step through line by line.
